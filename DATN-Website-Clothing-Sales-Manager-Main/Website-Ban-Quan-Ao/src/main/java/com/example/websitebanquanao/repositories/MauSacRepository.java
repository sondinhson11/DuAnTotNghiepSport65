...
public interface MauSacRepository extends JpaRepository<MauSac, Integer> {
    @Query("select new com.example.websitebanquanao.infrastructures.responses.MauSacResponse(m.id, m.ten,m.ngay_sua,m.ngay_tao,m.trang_thai) from MauSac m where m.trang_thai = 1 ORDER BY m.ten")
    public List<MauSacResponse> getAll();

    @Query("select new com.example.websitebanquanao.infrastructures.responses.MauSacResponse(m.id, m.ten,m.ngay_sua,m.ngay_tao,m.trang_thai) from MauSac m ORDER BY CASE WHEN m.trang_thai = 1 THEN 0 ELSE 1 END, m.ten")
